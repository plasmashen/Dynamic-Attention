    ...
    def forward(
        ...
        if top:
            ...
            if psl:
                ...
                extended_attention_mask[:, :, :, 0] = -1e9 * torch.ones([batch, 1, 1], dtype=torch.float32,
                                                                        device=attention_mask.device)
            ...
            attention_mask_t = (
                    (extended_attention_mask == 0)[:, :, :, past_key_value_length:].permute(0, 1, 3, 2) * (
            ...
            if top[0] == 0:
                idx = attention_scores_t.sum(1).sum(1).sort(dim=-1).indices[:, -top[1]:]
            else:
                idx = attention_scores_t.sum(1).sum(1).sort(dim=-1).indices[:, -top[1]:-top[0]]

            if decay_value == 0:
                # replace_val = -1e9 * torch.ones([idx.shape[0], 1, 1, top[1] - top[0]], dtype=torch.float32,
                #                                 device=attention_mask.device)
                # attention_mask = torch.scatter(attention_mask, -1, idx.unsqueeze(1).unsqueeze(1), replace_val)
                attention_decay = torch.ones_like(attention_scores)
                attention_decay[:, :, :, idx[0]] *= 0.
                attention_scores += attention_mask
                attention_probs = nn.Softmax(dim=-1)(attention_scores) * attention_decay
            else:
                attention_decay = torch.ones_like(attention_scores)
                attention_decay[:, :, :, idx[0]] *= decay_value
                attention_scores += attention_mask
                attention_probs = nn.Softmax(dim=-1)(attention_scores) * attention_decay
...
class AlbertLayerGroup(nn.Module):
    ...
    def forward(
        ...
        for layer_index, albert_layer in enumerate(self.albert_layers):
            layer_output = albert_layer(hidden_states,
                                        attention_mask,
