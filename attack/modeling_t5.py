...
class T5ForTextToText(torch.nn.Module):
    ...
    def from_pretrained(cls, name_or_path):
        ...
        if name_or_path in TEXTATTACK_MODELS:
            ...
            t5.model = T5ForConditionalGeneration.from_pretrained(
                name_or_path
            )
    ...
    def forward(
        ...
        if decay_value != 1:
            ...
            if scores.shape[2]!=scores.shape[3]:
                attention_scores_t = nn.Softmax(dim=-1)(mask + scores)[:,:,-1:,:]
            else:
                mask_ = mask
                mask_[:, :, :, 0] = -1e9 * torch.ones([batch, 1, 1], dtype=torch.float32, device=mask.device)
            ...
            attn_weights = nn.functional.softmax(scores.float(), dim=-1).type_as(
                scores) * attention_decay  # (batch_size, n_heads, seq_length, key_length)
        else:
            scores += position_bias
            attn_weights = nn.functional.softmax(scores.float(), dim=-1).type_as(
