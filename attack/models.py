...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, epsilon=0):
        super().__init__()
    ...
    def forward(
        ...
        inputs_embeds += self.epsilon * torch.rand_like(inputs_embeds) - self.epsilon / 2
        ...
        if attention_probs is None:
            ...
            if top and decay_value != 1:
                ...
                if "pre_seq_len" in self.config.__dict__:
                    psl1 = self.config.pre_seq_len
                if 'prefix' in self.config._name_or_path:
                    psl2 = self.config.pre_seq_len

                extended_attention_mask = torch.cat([attention_mask[:, :, :, 1:],
                                                     -1e9 * torch.ones([batch, 1, 1, 1], dtype=torch.float32,
                                                                       device=attention_mask.device)], dim=-1)

                extended_attention_mask[:, :, :, :psl1 + 1] = -1e9
                # past_key_value_length = psl if past_key_value is not None else 0
                # Mask prefix token, cls token, sep token and pad token
                # attention_scores_t = ((extended_attention_mask == 0)[:, :, :, past_key_value_length:].permute(0, 1, 3, 2) * (
                #                     extended_attention_mask == 0)) * attention_scores
                # attention_scores_t = attention_scores + extended_attention_mask
                # attention_probs_t = nn.Softmax(dim=-1)(attention_scores_t)
                # Attention modification
                attention_mask_t = ((extended_attention_mask == 0)[:, :, :, psl2:].permute(0, 1, 3, 2) * (
                ...
                if decay_value != 1:
                    attention_probs[:, :, :, idx[0]] *= decay_value
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        # p = hidden_states.abs().softmax(2)
        # for b in range(hidden_states.shape[0]):
        #     for i in range(1):
        #         id = np.random.choice([k for k in range(hidden_states.shape[2])], hidden_states.shape[2],
        #                               p=p[b, i].cpu().numpy())
        #         id = set(id)
        #         for j in range(hidden_states.shape[2]):
        #             if j in id:
        #                 hidden_states[b, i, j] *= 1 / (1 - (1 - p[b, i, j]) ** hidden_states.shape[2])
        #             else:
        #                 hidden_states[b, i, j] = 0
        return hidden_states
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if not top and not random_top:
                    layer_outputs = layer_module(
                        hidden_states,
                        attention_mask,
                        attention_probs[i] if attention_probs else attention_probs,
                        layer_head_mask,
                        encoder_hidden_states,
                        encoder_attention_mask,
                        past_key_value,
                ...
                # else:
                #     batch = attention_mask.shape[0]
                #     psl = self.config.pre_seq_len
                #     extended_attention_mask = torch.cat([attention_mask[:, :, :, 1:],
                #                                          -1e9 * torch.ones([batch, 1, 1, 1], dtype=torch.float32,
                #                                                            device=attention_mask.device)], dim=-1)
                #     extended_attention_mask[:, :, :, :psl + 1] = -1e9 * torch.ones([batch, 1, 1, psl + 1],
                #                                                                    dtype=torch.float32,
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, epsilon=0, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config, epsilon)
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, epsilon=0):
        ...
        self.bert = BertModel(config, epsilon)
        classifier_dropout = (
